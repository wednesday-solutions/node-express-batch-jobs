...
describe('init', () => {
  ...
  it('should start the server and listen for /grapqhl', async () => {
    ...
    expect(mocks.app.use.mock.calls[1][0]).toEqual(expect.any(Function));
    expect(mocks.app.use.mock.calls[2][0]).toEqual('/graphql');
    expect(mocks.app.use.mock.calls[3][0]).toEqual('/');
  });
